...
func main() {
	...
	stopPresence := buspresence.StartPresence(os.Getenv("COFISWARM_BRIDGE_URL"), "infer-sglang", map[string]any{"name": "infer-sglang"})

	httpSrv := &http.Server{Addr: *addr, Handler: mux}
	go func() {
		log.Printf("infer-sglang metadata on %s", *addr)
		if err := httpSrv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("infer-sglang: server error: %v", err)
		}
	}()
	...
	defer stop()
	<-ctx.Done()
	log.Printf("infer-sglang: shutting down")
	stopPresence() // carrier goodbye -> offline
	shutCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if err := httpSrv.Shutdown(shutCtx); err != nil {
		log.Printf("infer-sglang: graceful shutdown: %v", err)
	}
}
